<?php
	...
	$conexion = conectar();
	if(isset($_SESSION["nc"]) && isset($_SESSION["receptor"]) && isset($_POST["mensaje"])){
		$receptor = $_SESSION["receptor"];
		$contenido = $_POST["mensaje"];
		$destinatarios = explode(",", $receptor);
		$i = 0;
		while($i < count($destinatarios)){
			$query = "INSERT INTO chat (emisor, receptor, contenido) VALUES ('".$_SESSION['nc']."','".$destinatarios[$i]."','".$contenido."')";
